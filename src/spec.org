...
* Introduction
  ...
  The Transaction Executor algorithms   determine several   critical   aspects   of
...
** Pseudo-code Semantics
   ...
   behavior of the Transaction Executor parts.
...
* EverScale Platform Architecture
  ...
  The platform overall architecture is depicted on Fig. [[fig:general]]

   #+CAPTION: EverScale blockchain high-level architecture
   #+NAME:   fig:general
   [[./img/general.svg]]
...
** Remark
  ...
  Transaction Executor logic. Hence, it is the message that is being executed, not
...
** Multicurrency payments
   ...
   EverScale has an ability to work with other types of coins.  While
...
** Hashing Algorithm
   ...
   Transaction Executor uses  hashing in  several places  to compactly  store data
...
** Accounts
   ...
   its /address/: it is when we  put together the workchain identifier and its
   account identifier. See Fig. [[fig:account]].
   ...
   account on Fig. [[fig:account_lifecycle]].

   #+begin_src plantuml :results silent :file "../img/account_lifecycle.svg"
     skinparam backgroundColor transparent
     hide empty description
     left to right direction
     [*] -> Nonexistent
     Nonexistent --> Uninit : send\nvalue
   ...
   #+NAME: fig:account_lifecycle
   [[./img/account_lifecycle.svg]]

   In the beginning,  the account remains in an  /Nonexistent/ state -
   it stores no funds and no data.  Only after you send some value to
   the account, it  switches to the /Uninit/ state.  In  that state it
   is not yet very useful: it may only receive coins from the incoming
   messages.
   ...
   action. After the deletion, the account state is switched back to
   /Nonexistent/ state.
...
*** Account Address
    ...
    The location of an account in EverScale blockchain is represented as a
    two-value structure: the workchain number and the account identifier.
    This structure is called an /account address/. Hereinafter, we just
    call it /address/ for greater convenience.
...
**** Account State
     ...
     The account state defines the  mode of operation for the account,
     during  the   message  being  executed  for   that  account.  The
     Transaction Executor logic vary greatly  depending of what is the
    ...
    The life cycle of an account is depicted on Fig. [[fig:account_lifecycle]].

    Let us clarify the fields of enumeration items.

    For the  /AccountActive/, the  value /state_init/ defines the byte-code
    and data of the associated smart-contract. The field /init_code_hash/
    defines the hash of the  field /state_init.code/ that was used at  the
    moment of the account  initialization, or at the moment of the account
    freeze.

    In EverScale, it  is possible to change  the smart-contract's code
    on  the  fly  using  the   SetCode  action.   However,  the  value
    /init_code_hash/ stays  unaffected.
...
**** Smart-Contract Storage (StateInit)
    The byte-code and data of a contract are stored within a structure
    called /StateInit/.  Its name may  seem quite confusing.  It could
    have been named just State. The Init part comes from the fact that
    this structure is  also used for the initialization  of an account
    ...
    - /code/ and /data/ fields encodes the current byte-code and data of a
      contract. Here, data denote values of contract's variables.
...
** Messages
   <<messages>>
   In EverScale, smart-contracts communicate between each other and with
   non-blockchain applications by means of an asynchronous message passing.
...
*** Events
   ...
   - Transaction Executor automatically assigns the source address /src/ to be equal to
...
** Transaction Executor
   ...
   In case of success, Transaction Executor outputs the newly created transaction and
   the updated account.
   ...
   of the  Transaction Executor internals  that may  be easily  discarded. Message
   ...
   #+CAPTION: Transaction Executor Message Processing General Scheme
   #+NAME:   fig:phases
   [[./img/phases.svg]]
...
*** Transaction Executor Types
    ...
    For each type  of messages, there exists a  separate Transaction Executor.  In
...
*** Main Entry Point
   ...
   | account_root | Account record serialized in a form of Cells [[account]]  |
   | params       | Transaction Executor parameters [[parameters]]                        |
...
<<blockchain_config>>
    ...
    Besides  ExecuteParams, the  Transaction Executor  relies on  BlockchainConfig
    parameters.  They are passed  implicitly, at the Transaction Executor creation
    time.
...
*** Special Smart-Contracts
    ...
    Transaction Executor has to apply all those conditions.
...
*** Error Code Enumeration
    When Transaction Executor  encounters an  error during message  processing, it
...
*** Transaction Description Object
    ...
    During the incoming message processing, Transaction Executor constructs the report
...
** Storage Phase
   ...
   *_Input:_*
   ...
   - /msg_balance/ - message balance, has type /Grams/
   - /acc_balance/ - current balance of the account, has type /Grams/
   - /config/ - main blockchain parameters, has type [[blockchain_config][BlockchainConfig]]

   *_Output:_*

   This phase always succeeds. The return values may differ:
...
*** Account From Message Algorithm
    ...
    [[compute_new_state][compute_new_state]] algorithm.

    *_Input_*
    - msg: incoming message being processed, has type [[messages][Message]]
    - msg_remaining_balance: the current amount of coins left on the message balance, has type /Uint/

    *_Output_*
    - Either returns a new [[account][Account]] object, or None. Both results are considered successful.
...
** Action Phase
   ...
   *_Input_:*
   ...
   - /account/ - account executing the message, has type [[account][Account]]
...
<<bounce_phase>>
   ...
   *_NOTE_:*
   - Function /get_fwd_prices()/ was defined [[get_fwd_prices][here]].
