...
** Pseudo-code Semantics
   ...
   - The call $obj.clone()$ does the deep copy of the object
   ...
   - We use *is* operator to do type test. For example, to test that message is internal, we
...
* EverScale Platform Architecture
  ...
  Smart-contracts are programs that operate user valuable assets
  on their behalf. Valuable assets are usually cryptocurrency tokens
  or some digital goods, like NFTs.

  Smart-contract execution is triggered by a message sent from some
  other party. If the message was delivered from the outside world (i.e.
  from the user program), it is said to be /external/. Otherwise, the message
  is considered /internal/.

  Smart-contracts may also generate log records called /events/.
  Those records are used as information signals for an external observers.
  They foster communication between smart-contracts and off-chain programs.
...
** Platform Implementation
   ...
   transaction record that gets sealed  into the block (the next block
   of a chain).
...
** Multichain Architecture
   ...
   identifier in a range  -1 ... 255, the values -1  and 0 are already
...
** Multicurrency payments
   ...
   EverScale has an inherent support  for multiple coins. While system
   payments like  gas and storage  fees are  made only in  Evers, the
   other value transfers may contain coins of other currencies. This
   contrasts with most of other blockchains where there is only a single
   native cryptocurrency, and other currencies may be made only using
   artificial token smart-contracts.

   Currently, this  feature is not  used widely. In this  document, we
   limit  our specification  effort  only  for the  case  of a  single
   currency - Evers. This choice significantly simplifies the business
   logic of the execution handlers.
...
#+BEGIN_COMMENT
   ...
   Here, /Option<T>/ denotes the algebraic datatype with constructors:
   /None/ and /Value<T>/.

   For an active account, /stuff/ field is initialized
   with the value of type /AccountStuff/.

   If the /stuff/ field is set to  /None/, it is said that the account
   does not  exist. Please note  that this 'non-existing' status  is a
   mere optimization.  From user  standpoint, non-existing  account is
   indistingushable from uninitialized account. We will not distinguish
   those states either.
...
*** Account Address
    ...
    The type /Number9/ denotes natural numbers in a range $0 ... 2^{9} - 1$.

    The type /SlideData/ denotes a binary blob encoded in a tree data structure.

    The type /i32/ is a 32-bit integer.
...
*** Account Storage
    ...
    occupying space, on a regular basis. The fee depends on the size of
    data being  stored, the  current prices and  when the  last charge
    took  place. In  some circumstances,  an account  may also  have a
    debt,  called due  payment. All  this information  (except current
    storage prices) is stored in the /StorageInfo/ structure.
    ...
    | Field       | Description                                                 |
    |-------------+-------------------------------------------------------------|
    | used        | The amount of bits and cells stored occupied by the account |
    | last_paid   | Time of the latest payment, expressed as Unix Epoch         |
    | due_payment | Debt for the account                                        |


    The amount of storage used by the account is encoded with /StorageUsed/.
    \\
    \\
...
**** Account State
    ...
    hash of a state_init that was used at the moment of account
    initialization. During the initialization, this value should be
    equal to a hash of code and data of a smart-contract for this account.

    It is possible to change both code and data for the account, but this
    value will not change. This value defines the address of a smart-contract
    within the current workchain.

    It is the same for /AccountFrozen/. The value of /state_init_hash/
    defines the hash of the smart-contract /state/ at the moment of a freeze.
...
**** Smart-Contract Storage (StateInit)
    ...
    The /split_depth/ field was initially devoted to large multi-shard
    smart-contracts, but currently it is not used.

    The /special/ fields signals the fact that the smart-contract is related
    to the blockchain system functioning. This is related to very small amount
    or contracts residing in the Masterchain, i.e. Elector, Config, Giver, etc.
    There is a special logic of executing messages destined to those contracts.

    The fields /code/ and /data/ encodes the byte-code and the data of a contract.

    The field /library/ encodes the code libraries the contract may refer to its
    code. Current status of this the library function is unclear.

** Messages
   <<messages>>
   Smart-contracts interact with each other by sending and receiving /messages/.

   A message is a data block that encodes:
   ...
   The last two fields /body_to_ref/ and /init_to_ref/ are not used in the Executor, hence
...
*** External Message
   ...
   The fields /src/  and /dst/  are source and destination addresses.
   ...
   The field /import_fee/ defines the commission for .. ?
...
*** Events
   ...
   Executor automatically assigns the source address as the smart-contract
   address emitted the event.

   The destination address /dst/ may contain any identifier. It is included
   solely for information purposes. For example, in /TON Solidity/ language,
   the /emit/ operator may be equipped with destination address parameter.
...
** Parameters
   ...
   | Field          | Description                                                                         |
   |----------------+-------------------------------------------------------------------------------------|
   | state_libs     | A set of references to external libraries. This mechanism is not support currently. |
...
** Transaction
   ...
   | orig_status     | Status of the account upon receiving the message  |
   | end_status      | Status of the account after executing the message |
...
** Executor
   ...
   module. Other entry  points are were either  flagged as deprecated,
   ...
   of the Executor internals that may be easily discarded. Message execution
   phases are a part of EverScale smart-contracts programming architecture.
   It is assumed that you have a good grasp on it, to be able to do proper
   troubleshooting in case something is not working as expected. Without
   this knowledge, it may be challenging to debug the problem.
...
*** Executor Types
    ...
    mentioned, there  are also a  special type  of messages that  is a
    part  of  a  wider  protocol.   For  example,  TickTock  messages,
    SplitMerge messages, etc.

    There  are  several  Executors  available  for  different  message
    categories.     In   this    work,    we    consider   only    the
    OrdinaryTransactionExecutor,      that      is     defined      in
    /orderinary_transaction.rs/.  Other  types  of Executors  are  not
    considered here.
...
*** Code Execution Fee
    ...
    coins attached to  the message, initiated the call,  but there are
    nuances.
...
*** Message Passing Fee
    ...
    Validators do  the work  on message  passing. To  compensate their
...
*** Data Storage Fee
    ...
    moment, measured in seconds.  The fee gets charged on each message
    processing [[storage_phase]].
...
**** Data Storage Fee Calculation
    ...
    genesis block of the blockchain, and $t_0$ is a time moment of
    those initial prices being set.

    Let $now$ denote the current  time moment, i.e. the moment of time
    when we want to calculate the storage fee.
    It is always greater or equal than the most recent price change.

    Let  $last\_paid$ denote  the time  moment of  the latest  storage
    payment. If the payment didn't take place, $last\_paid = 0$.

    To simply  the calculation form,  let us introduce a new list
    $Pr'$, such that:

    $$Pr' = \langle  (pr_k, t'_k), ..., (pr_N, t'_N)  \rangle $$ where
    $t'_k$ is the  least timestamp among values $t_1 ...  t_N$ that is
    /greater/ than  $last\_paid$.
    $$t'_k = min \{ t_i | t_i \in T \land t_i > last\_paid \}$$

    In  other words, the  values $t'_k,
    ...,  t'_N$ form  a subset  of $T$  where each  value is  strictly
    greater than $last\_paid$.
    ...
    Here, we  use shortcuts:  $pr_i = fst(Pr''_i)$ -
...
**** Data Storage Fee Calculation Algorithm
    ...
    We provide the pseudo-code for the algorithm /calc_storage_fee/, implemented in imperative fashion.
    It resemble how it is implemented in the node.

    *_Input:_*
    - storage_info - the account storage info struct, has type AccStorageInfo.
    ...
    *_Output_*:
    ...
    - storage_info - updated account storage info, has type AccStorageInfo
...
*** Special Smart-Contracts
    In   the  BlockchainConfig   parameters,   there  is   a  set   of
    smart-contract  addresses   that  have   special  status   in  the
    system. Being system-oriented, validators are obligated to process
    their execution in a special priveledged manner.

    The special smart-contracts enjoy the following privilege:
    ...
    - The value /storage_state.last_paid/ is always equals 0

    Upon executing  message for  one of  those special  contracts, the
    Executor applies all those conditions.
...
*** RawConfig options
    ...
    /config_addr/   -  is the configuration smart-contract address,
...
*** Error Code Enumeration
    When Executor encounters an error during message processing, it returns a
    special answer to the calling side. The answer contains an error code.
    Here we list possible error codes and their short description. In out further
    discussion, we rely on those mnemonic names.
...
*** Transaction Description Object
    ...
    system invariants, critical for its safety.
    ...
    | bounce_ph    | Bounce phase descriptor                        |
...
**** Compute Phase Descriptor
    ...
    *Choice 1. Skipped*
    ...
    |----------+--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
    | NoGas    | Caused by the following conditions: 1) After Credit and Storage phases, the account balance had no coins: its balance equals zero; 2) Values /gas_limit/ и /gas_credit/, calculated with the [[init_gas][init_gas]] algorithm, both equals 0.                                                                                                                                                                                                                      |
    |----------+--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
...
**** Action Phase Descriptor
    ...
    | result_code       | Action phase failed with the result code, see [[action_result_codes]]                                  |
...
**** Action Result Codes
    ...
    | RESULT_CODE_NOT_ENOUGH_EXTRA          | Extra-tokens balance is insufficient to execute to action *link*                                                                    |
    ...
    1. /RESULT_CODE_UNKNOWN_OR_INVALID_ACTION/ reasons are:
       ...
       - Reserve action has invalid flags
         - Unknown flag is set
         - Flag RESERVE_PLUS_ORIG is set, but RESERVE_RESERVE isn't
    2. /RESULT_CODE_NOT_ENOUGH_GRAMS/ reasons are:
       ...
       - Message balance is insufficient to cover message delivery fees *link*
       - Account balance is insufficient to cover all message delivery fees *link*
...
**** Bounce Phase Transaction Descriptor
    ...
    - Ok(TrBouncePhaseOk) denotes success. Details of the phase are put into the parameter value
    ...
    *Choice 1. Nofunds*
    ...
    Here, /msg_size/ - the size of generated bounce message. This value is not used.
    The field /req_fwd_fees/ denotes the fee for the message delivery.
    ...
    *Choice 2. Ok(TrBouncePhaseOk)*
    ...
    - /msg_size/ not used.
    - /msg_fees/ the fee for *?*
    - /fwd_fees/ the fee for *?*
...
*** Action SendMsg
    ...
    This action has a lot of modes that can be combined using logical /OR/ operator.
...
* Message Processing Algorithm
  ...
  *_Input:_*
  - /in_msg/ - incoming message, has type /Message/ [[messages]]
  - /acc/ - account, has type /Account/ [[accounts]]
  - /params/ - executor parameters, has type /Parameters/ [[parameters]]
  - /config/ - blockchain configuration, has type /BlockchainConfig/ [[blockchain_config]]

  *_Output:_*

  On success, returns Ok(acc1, trans), such that:
    - /acc1/ - updated account, has type /Account/
    - /trans/ - transaction, has type /Transaction/
  ...
  *_Modifies:_*
  ...
  #+begin_src python
    def ExecuteMessage(in_msg, account, params, config):
        ...
        if in_msg.hdr is ExtOutMessageHeader:
            return ExecutorError.InvalidExtMessage
        if in_msg.hdr.dst == None:
            return ExecutorError.TrExecutorError()
        if config.block >= VERBLOCK_REVERT_ANYCAST:
            return ExecutorError.TrExecutorError()
        if in_msg.hdr is ExtInMessageHeader and account.balance == 0:
...
#+begin_src python
    def ExecuteInternalMessage(in_msg, account, params, config):
        ...
        if credit_first:
            ...
            if credit_ph_res == Ok(credit_ph):
                descr.credit_ph = credit_ph
            else:
                return ExecutorError.TrExecutorError()

        # Execute Storage Phase
        storage_ph_res = storage_phase(account, acc_balance, tr,
                                       is_masterchain, False)
        if storage_ph_res == Ok(storage_ph):
            descr.storage_ph = storage_ph
        else:
            return ExecutorError.TrExecutorError()
        ...
        compute_ph_res = compute_phase(in_msg, account, acc_balance, msg_balance,
                                       params.state_libs, smci, stack,
                                       is_masterchain, False, params.debug)
        ...
        if descr.compute_ph.success:
            act_phase_res = action_phase(tr, account, original_acc_balance,
                                         acc_balance, msg_balance, phase.gas_fees,
                                         actions, new_data, False)
        ...
        if (descr.aborted == True) and (in_msg.hdr.bounce = True):
            if descr.compute_ph is Vm:
                bounce_ph_res = \
                    bounce_phase(msg_balance, acc_balance, compute_gas_fees,
                                 tr, my_addr)
                if bounce_ph_res == Ok(bounce_ph, bounce_msg):
                    descr.bounce_ph = bounce_ph
                    if (bounce_msg != None):
                        out_msgs = out_msgs + [bounce_msg]
                else:
                    return ExecutorError.TrExecutorError()
            if descr.bounce_ph is Ok:
 ...
 The function /add_messages/ assigns the proper logical timestamp for each message from the out_msgs collection,
 and then include the message into the transaction.
...
#+begin_src python
  def ExecuteExternalMessage(in_msg, account, params, config):
      ...
      # Execute Storage Phase
      storage_ph_res = storage_phase(
          account, acc_balance, tr, is_masterchain, False)
      if storage_ph_res == Ok(storage_ph):
          descr.storage_ph = storage_ph
      else:
          return ExecutorError.TrExecutorError()
      ...
      if compute_ph_res == Ok(compute_ph, new_data):
          descr.compute_ph = compute_ph
      else:
          return ExecutorError.TrExecutorError()
      ...
      if descr.compute_ph.success:
          act_phase_res = action_phase(tr, account, original_acc_balance,
                                       acc_balance, msg_balance, phase.gas_fees,
                                       accounts, new_data, False)
          if act_phase_res == Ok(action_ph, msgs):
              descr.action = action_ph
              out_msgs = msgs
          else:
              return ExecutorError.TrExecutorError()
...
*** Compute Phase Success Conditions
    ...
    considered successful. The success status is important, because it
    decides if the action phase will be executed afterwards.

    For the phase to fail, one of the following conditions must hold:
    1. The value returned is not equal to /Ok(TrComputePhase.Vm(vm_phase), _, _)/
    2. The smart-contract data is not committed after the execution [fn:5]
    3. The new smart-contract data is ill-formed
    4. The generated actions list is ill-formed
...
*** Compute Phase Exit Code
    ...
    custom  nature.  If  the  exception  is a  system  one, i.e.   not
    intentionally inspired by the code,  the exit code contains one of
...
*** Calculate Gas Fee Algorithm
    ...
    *_Input:_*
     - gas_prices - a structure with actual gas prices, has type /GasLimitsPrice/ [[gas_prices]]
      ...
      def compute_new_state(account, acc_balance, in_msg):
          if account.status == AccountStatus.AccStateNonexist:
              ...
              if acc_balance != 0 and in_msg.state_init != None:
                  if account.try_activate_by_init_code_hash(in_msg.state_init) == Ok:
                      return None
                  else:
                      return ComputeSkipReason.BadState
              return ComputeSkipReason.NoState
...
*** Activate By Init Algorithm
    ...
    *_Modifies:_*
    - /account/ - the field /stuff.storage.state/ gets updated by the state_init on success
    ...
    #+begin_src python
      def try_activate_by_init_code_hash(account, state_init):
          ...
          if stuff.storage.state == AccountState.AccountUninit:
              if hash(state_init) == stuff.addr.address:
                  new_state = AccountState.AccountActive(
                      hash(state_init.code), state_init
                  )
              else:
                  return Err
          elif stuff.storage.state == \
              ...
              new_state = stuff.storage.state

          account.stuff.storage.state = new_state
          return Ok
    #+end_src
...
** Action Phase
   ...
   *_Output_:*
   ...
   - /result_code/ describes the nature of an error, see [[action_result_codes][here]].
   ...
   #+begin_src python
     def action_phase(tr, account, original_acc_balance, acc_balance,
       ...
       for action in actions:
           if action == OutAction.SendMsg(mode, msg):
               if (mode & SENDMSG_ALL_BALANCE != 0):
                   out_msgs_tmp.push(msg)
                   continue
               ...
               if result == Ok:
                   ...
                   out_msgs_tmp.push(mode, out_msg)
               else:
                   return result
           elif action == OutAction.ReserveCurrency(mode, value):
               result = reserve_action_handler(mode, value, original_acc_balance,
                                               acc_remaining_balance)
               if result == Ok(reserved_value):
...
[fn:4] Well, at least, until the part of the chain residing the transaction gets cut-off to reduce the disk space consumption.
